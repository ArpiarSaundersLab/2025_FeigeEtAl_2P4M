...
cell_de <- read_rds("/Users/chingr/OHSU Dropbox/Saunders Lab's shared workspace/arpy/manuscripts/2023_Thai2P4M_FeigeYoung/ms_analyses/1_karl_analysis/Data/de/DE.tha5.vs.NI.CoarseCellType.rds")

celltype_de <- read_rds("/Users/chingr/OHSU Dropbox/Saunders Lab's shared workspace/arpy/manuscripts/2023_Thai2P4M_FeigeYoung/ms_analyses/1_karl_analysis/Data/de/DE.tha5.vs.NI.CellType.rds")
...
astro_de <- subset(cell_de, cell_type == 'Astrocyte')
neuron_de <- subset(cell_de, cell_type == 'Neuron')
...
sig_gene_astro <- subset(astro_de, p_val_adj <= 0.01)
sig_gene_neuron <- subset(neuron_de, p_val_adj <= 0.01)
...
```{r}
# KEGG Pathway M9809 (Cytokine Cytokine Receptor Interactions)
...
# Neurons: 
neuron_cyto <- intersect(sig_gene_neuron$gene,cytokines.df$cytokines)
...
# Astrocytes:
astro_cyto <- intersect(sig_gene_astro$gene,cytokines.df$cytokines)
...
# Plots Relevant Cytokine Genes
plot_astro <- ggplot(data = astro_de, aes(x = avg_logFC, y = -log10(p_val_adj),
                                          label=if_else(
                                          gene %in% astro_cyto,
                                          gene, NA))) + 
        ...
        ylim(-5, 87) +
        geom_point(data = subset(astro_de, gene %in% astro_cyto), color = "red", size = 3) +
        ggrepel::geom_text_repel(size = 3, min.segment.length=0.5, force=4, max.overlaps=80)

plot_neuron <- ggplot(data = neuron_de, aes(x = avg_logFC, y = -log10(p_val_adj),
                                            label=if_else(
                                          gene %in% neuron_cyto,
                                          gene, NA))) + 
    geom_point(color = 'black', size = 2.5) + 
        xlim(-7, 7) +  # Set x-axis limits
        ylim(-5, 87) +
    geom_point(data = subset(neuron_de, gene %in% neuron_cyto), color = "red", size = 3) +
    ggrepel::geom_text_repel(size = 3, min.segment.length=0.5, force=4, max.overlaps=80)
        ...
        theme(  panel.grid.major = element_line(color = "gray80", size = 0.5),
                panel.border = element_rect(color = "black", fill = NA, size = 0.5),
                 ...
                 axis.text.y = element_text(size = 25)) + 
                        labs(x = NULL, y = NULL)
       #  ggrepel::geom_text_repel(size = 6, min.segment.length = 1, force = 20, max.overlaps=80, nudge_x = 2, nudge_y = 15)
...
#3 Check known receptors that are expressed on microglia in Tha5 condition
```{r}
# Loads NI vs. Tha5 DE
mg_de <- read_rds("/Users/chingr/OHSU Dropbox/Saunders Lab's shared workspace/arpy/manuscripts/2023_Thai2P4M_FeigeYoung/ms_analyses/2_greg_analysis/Data/de/mg_ni_vs_tha5_de.RDS")
...
# known receptors for upregulated cytokines. 
# Commented out genes are not expressed at detectable levels in microglia
FeaturePlot(mg_so, features = c("CCR1"), reduction = "umap")
# FeaturePlot(mg_so, features = c("CCR2"), reduction = "umap")
FeaturePlot(mg_so, features = c("CCR5"), reduction = "umap")
# FeaturePlot(mg_so, features = c("CCR11"), reduction = "umap")
# FeaturePlot(mg_so, features = c("CXCR1"), reduction = "umap")
FeaturePlot(mg_so, features = c("CXCR2"), reduction = "umap")
# FeaturePlot(mg_so, features = c("CXCR3"), reduction = "umap")
FeaturePlot(mg_so, features = c("ACVR1B"), reduction = "umap")
FeaturePlot(mg_so, features = c("ACVR2B"), reduction = "umap")
FeaturePlot(mg_so, features = c("ACVR2A"), reduction = "umap")
FeaturePlot(mg_so, features = c("KIT"), reduction = "umap")

allgenes <- FeaturePlot(mg_so, features = c("CCR1","KIT","CCR5","CXCR2","ACVR1B","ACVR2B","ACVR2A","KIT"), reduction = "umap")

wrap_plots(allgenes)

cyto_genes <- c('CXCL10','CCL3','CCL8')

mg_ligand <- VlnPlot(tha.seurat, features = cyto_genes, group.by = "Experiment", pt.size = 0.1, stack = FALSE)


```

# Global Expression of Cytokines and Receptors
```{r}
# Load full dataset
# tha_seurat <- read_rds("/Users/chingr/OHSU Dropbox/Saunders Lab's shared workspace/arpy/manuscripts/2023_Thai2P4M_FeigeYoung/ms_analyses/1_karl_analysis/Data/seurat_obj/tha.seurat.RDS")
...
# plot relevant  datasets
cyto_receptor <- c("CCR1","KIT","CCR5","CXCR2","ACVR1B","ACVR2B","ACVR2A","KIT","CXCR3")

allgenes_all_cells <- FeaturePlot(tha.seurat, features = cyto_receptor, reduction = "umap")

wrap_plots(allgenes_all_cells)

vln_receptor <- VlnPlot(tha.seurat, features = c('CCR1','CCR5'), group.by = "CoarseCellType", pt.size = 0.1, stack = FALSE)
vl_test <- VlnPlot(tha.seurat, features = c('CCR1','CCR5','CCR2', 'CCR3'), group.by = "CoarseCellType", pt.size = 0.1, stack = FALSE)


# plot signaling molecules

cyto_ligand <- c("CXCL10","INHBA","KITLG","CCL2","CCL3","CCL4","CCL8","CXCL8","CXCL10","KIT","IL6ST")
cyto_ligand_2 <- c("CCL2", "CCL3","CCL4","CCL8","CCL4L2", "CXCL8", "CXCL10")


allcyto_all_cells <- FeaturePlot(tha.seurat, features = cyto_ligand, reduction = "umap")

wrap_plots(allcyto_all_cells)

vln_ligand <- VlnPlot(tha.seurat, features = cyto_ligand, group.by = "CoarseCellType", pt.size = 0.1, stack = FALSE)

ligand_heatmap <- DoHeatmap(tha.seurat, features = cyto_ligand, group.by = 'CoarseCellType') +
  scale_fill_gradientn(colors = c("white", "blue"))  # Optional color scale''
...
ligand_heatmap_2 <- DoHeatmap(tha.seurat, features = cyto_ligand_2, group.by = 'CoarseCellType') +
  scale_fill_gradientn(colors = c("white", "blue")) +
  
          theme(legend.position = "right",               # keep expression legend
...
# ggsave(paste0(save_path,"heatmap.png"),
      #  plot = ligand_heatmap_2,
       # width = 12, height= 5, units='in', 
       # dpi = 600, device = "png")

```
### cytokine and receptor heatmaps
```{r}
...
# plots CCR1 and CCR5 only acros cell types
desired_order <- c("Microglia", "Astrocyte", "Neuron", "hNSC", "Mitotic")
tha.seurat$CoarseCellType <- factor(tha.seurat$CoarseCellType, levels = desired_order)
my_colors = c(Microglia_color, Astrocyte_color, Neuron_color, hNSC_color ,Mitotic_color)


CCR1_plot <- VlnPlot(tha.seurat, features = c('CCR1'), group.by = "CoarseCellType", pt.size = 0.1, stack = FALSE,  cols = my_colors) +
                        theme(legend.position = "none",
                        plot.title = element_text(size = 26, family = 'Arial', face = "italic", hjust = 0.5, color = "black"),
                        axis.title.x = element_text(),  # X-axis label
                        axis.title.y = element_text(size = 21),  # Y-axis label
                        # axis.text.x = element_text(family = "Arial", face = "bold.italic", size = 18, angle = 90, hjust = 1),   # X-axis tick labels
                        axis.text.x = element_blank(),   # X-axis tick labels
                        axis.text.y = element_text(size = 25),
...
CCR5_plot <- VlnPlot(tha.seurat, features = c('CCR5'), group.by = "CoarseCellType", pt.size = 0.1, stack = FALSE,  cols = my_colors) +
                        theme(legend.position = "none",
                        plot.title = element_text(size = 26, family = 'Arial', face = "italic", hjust = 0.5, color = "black"),
                        axis.title.x = element_text(),  # X-axis label
                        axis.title.y = element_text(size = 21),  # Y-axis label
...
ggsave(paste0(save_path ,"CCR1_cells.png"),
       plot = CCR1_plot,
       width = 3.5, height= 4, units='in', 
       dpi = 600, device = "png")


ggsave(paste0(save_path,"CCR5_cells.png"),
       plot = CCR5_plot,
       width = 3.5, height= 4, units='in', 
       dpi = 600, device = "png")
...
# Microglia Expression of Cytokines
```{r}
...
select_lig <- c('CCL3', 'CCL4', 'CCL4L2', 'CCL5', 'CCL8',
                                                   'CXCL8', 'CXCL10', 'CXCL16', 'IL1A', 'IL1B', 'IL10', 'IL18',
                                                   'CSF1', 'INHBA')
...
# for (i in (12)) {
 for (i in (1:length(cytokines.df[[1]]))) {
  ...
  # ANOVA
      ...
      y.position = seq(from = max(df$expression) * 1.05, length.out = nrow(sig_comparisons), by = 0.2)  # spread brackets vertically
        )
...
# plot <- mg_cyto_gene + stat_pvalue_manual(sig_labels,
#                       xmin = "group1", 
#                       xmax = "group2", 
#                       label = "label", 
...
across_cells_CCR <- VlnPlot(tha.seurat, features = c('CCL3','CCL8'), split.by = 'Experiment', group.by = "CoarseCellType", pt.size = 0.1, stack = FALSE)
